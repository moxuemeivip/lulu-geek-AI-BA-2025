...
print(np.array(daily_return["上证综指"]))

import numpy as np
#model.fit(daily_return["上证综指"].reshape(-1, 1), daily_return["探路者"].reshape(-1, 1))
model.fit(np.array(daily_return["上证综指"]).reshape(-1,1), np.array(daily_return["探路者"]).reshape(-1,1))
pred_y = model.predict(np.array(daily_return["上证综指"]).reshape(-1,1))
print(model.coef_)
"""